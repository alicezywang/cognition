#include "mobilenet_ssd/tensorrt/pluginImplement.h"
#include "mobilenet_ssd/tensorrt/mathFunctions.h"
#include <vector>
#include <algorithm>
/******************************/
// PluginFactory
...
nvinfer1::IPlugin* PluginFactory::createPlugin(const char* layerName, const nvinfer1::Weights* weights, int nbWeights)
{
    ...
    else if (!strcmp(layerName, "mbox_conf_reshape"))
    {
        ...
        mMbox_conf_reshape = std::unique_ptr<Reshape<7>>(new Reshape<7>());
...
IPlugin* PluginFactory::createPlugin(const char* layerName, const void* serialData, size_t serialLength)
{
    ...
    else if (!strcmp(layerName, "mbox_conf_reshape"))
    {
        ...
        //num of class,by lcg
        mMbox_conf_reshape = std::unique_ptr<Reshape<7>>(new Reshape<7>(serialData, serialLength));
        return mMbox_conf_reshape.get();
    }
    //softmax
    else if (!strcmp(layerName, "mbox_conf_softmax"))
