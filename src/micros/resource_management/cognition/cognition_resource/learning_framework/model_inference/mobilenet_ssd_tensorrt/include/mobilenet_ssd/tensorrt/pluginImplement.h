...
public:
    ...
    Dims getOutputDimensions(int index, const Dims* inputs, int nbInputDims) override
    {
        ...
        assert((inputs[0].d[0])*(inputs[0].d[1]) % 7 == 0);
...
//        CHECK(cudaMemcpyAsync(outputs[0],inputs[0],batchSize*mCopySize*sizeof(float),cudaMemcpyDeviceToDevice,stream));
        ...
        //cudaSoftmax( 8732*21, 21, (float *) *inputs, static_cast<float *>(*outputs));//for ssd
        cudaSoftmax( 1917*7, 7, (float *) *inputs, static_cast<float *>(*outputs));//for mobilenet ssd
...
public:
    ...
    //reshape layer
    std::unique_ptr<Reshape<7>> mMbox_conf_reshape{ nullptr };
    //flatten layers
