...
DetectionTFImpl::DetectionTFImpl()
{
    string name = "RCNN";

    if(name == "RCNN"){
        ml_model = boost::make_shared<machine_learning::FasterRCNNModel>();
        cout <<"~~~ RCNN Task Starting ~~~"<< endl;
    }
    else if (name == "MASK") {
        //ml_model = boost::make_shared<machine_learning::MaskRCNNModel>();
        cout <<"~~~ RCNN MASK Starting ~~~"<< endl;
    }
 ...
 */
bool DetectionTFImpl::call(vector<boost::any>& inputs, vector<boost::any>& results)
{
...
PLUGINLIB_EXPORT_CLASS(cognition_bus::DetectionTFImpl, cognition_bus::SoftBusBase)
